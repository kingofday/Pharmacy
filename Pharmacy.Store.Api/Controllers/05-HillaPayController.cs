    ...
    public class HillaPayController : ControllerBase
    {
        ...
        public async Task<IActionResult> AfterGateway([FromForm] HillaPayAfterGatewayModel model)
        {
            if (model.Status.status != 400)
                return Redirect($"{_settings.ShowPaymentResult.ReactUrl}0/1/0/{model.result_transaction_callback.transaction_id}");
            else
            {
                var findPayment = await _paymentSrv.FindAsync(model.result_transaction_callback.transaction_id);
                if (!findPayment.IsSuccessful)
                    return Redirect($"{_settings.ShowPaymentResult.ReactUrl}0/1/0/{model.result_transaction_callback.transaction_id}");
                var verify = await _orderSrv.Verify(findPayment.Result, new object[1] { model.result_transaction_callback.rrn });
                return Redirect($"{_settings.ShowPaymentResult.ReactUrl}{Convert.ToByte(verify.IsSuccessful)}/{(byte)findPayment.Result.Type}/{verify.Result.OrderUniqueId}/{verify.Result.TrackingId}");
            }
