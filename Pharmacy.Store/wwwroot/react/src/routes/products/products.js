...
class Products extends React.Component {
    ...
    async _fetchDrugs() {
        ...
        this.setState(p => ({ ...p, loading: false, maxPrice: get.result.maxPrice, items: get.result.items }));
    }
                    ...
                    <Row id='first-row' className='mb-15'>
                                ...
                                <Col xs={12} className='mb-15'>
                                    <FormControl variant="outlined" className='card'>
                                        <Select
                                            ...
                                            {Object.keys(sorts).map((k) => <MenuItem value={sorts[k].value}>{sorts[k].desc}</MenuItem>)}
                                        </Select>
                                    </FormControl>
                                ...
                                <Col xs={12}>
                                    <Row>
                                        {this.state.loading ? [0, 1, 2, 3, 4, 5, 6, 7, 8].map((x, idx) => (<Col key={idx} xs={12} sm={4}><DrugItem loading={true} /></Col>)) :
                                            this.state.items.map((item, idx) => <Col key={idx} xs={12} sm={4}><DrugItem item={item} /></Col>)
                                        }
                                    </Row>
                        ...
                        <Col sm={4} className='d-none d-sm-block'>
                            <Row>
                                ...
                                <Col xs={12}>
                                    <PriceRange maxPrice={this.state.maxPrice} />
                                </Col>
                                <Col xs={12}>
