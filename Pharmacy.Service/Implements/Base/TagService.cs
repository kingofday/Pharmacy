...
namespace Pharmacy.Service
{
    public class TagService : ITagService
    {
        ...
        public PagingListDetails<Tag> Get(TagSearchFilter filter)
        {
            ...
            return _appUow.TagRepo.GetPaging(new PagingQueryFilter<Tag> { Conditions = conditions, PagingParameter = filter, OrderBy = x => x.OrderByDescending(x => x.Name) });
        }
        ...
        public PagingListDetails<Tag> GetForDashbord(TagSearchFilter filter)
        {
            ...
            return _appUow.TagRepo.GetPaging(new PagingQueryFilter<Tag> { Conditions = conditions, PagingParameter = filter, OrderBy = x => x.OrderByDescending(i => i.TagId) });
        }
